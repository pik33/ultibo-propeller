...
serialfront,serialtail:integer;
serialbuf:array[0..31] of byte;
r:integer;
filename:string;
i,fh:integer;
...
for i:=0 to 3 do
 ...
 constructor TSerialInput.Create(CreateSuspended : boolean);
...
procedure TSerialInput.execute
                               ;
var q:byte;
...
repeat
  if serialfront<>((serialtail-1) mod 32) then
    begin
    SerialRead(@q,1,Count);
    serialbuf[serialfront]:=q;
    serialfront+=1;
    serialfront:=serialfront mod 32;
    end;
until terminated;

end;

function serialread2:integer;

var r:integer;

begin
if serialfront<>serialtail then
  begin
  r:=serialbuf[serialtail];
  serialtail:=(serialtail+1) mod 32;
  end
else
  r:=-1;
...
if SerialOpen(1920000,SERIAL_DATA_8BIT,SERIAL_STOP_1BIT,SERIAL_PARITY_NONE,SERIAL_FLOW_NONE,0,0) = ERROR_SUCCESS then
  ...
  aserialinput:=tserialinput.create(true);
  aserialinput.start;
  Count:=0;
  Characters:='';
  ...
  while True do
    ...
    if r<>-1 then
       ...
       if r=$40 then
          ...
          repeat
            r:=serialread2;
            if r<>255 then filename+=chr(r)
          until r=255;
          ConsoleWindowWriteLn(WindowHandle,'file open request: '+filename);
          fh:=fileopen(filename,$40);
          if fh>0 then
             begin
             consolewindowwriteln(windowhandle,'file opened with fh='+inttostr(fh));
             fileclose (fh); //temp
             end;
          end;
       ConsoleWindowWriteLn(WindowHandle,'Received byte: '+inttostr(r));
       end;
    end;
  end;
